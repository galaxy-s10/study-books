export function pinyin(arr,parseitem) {
  ...
  // const str = ConvertPinyin(x)
  // return str
  var newarr = []
  arr.forEach((val, index) => {
    // 遍历数组，修改每一项对象里要转化的汉字属性为小写字母
    if(parseitem == 'friend.name'){
      var firstabc = ConvertPinyin(val.friend.name).charAt(0).toLowerCase()
    }else{
      var firstabc = ConvertPinyin(val.member.name).charAt(0).toLowerCase()
    }
    newarr.push({
      firstabc,
      ...val
  ...
  newarr.forEach((val, index) => {
    newarr.sort((a, b) => {
      // if (a.firstabc.toUpperCase() < b.firstabc.toUpperCase()) {
      //   return -1
      // }
      if (/^\d/.test(a.firstabc) ^ /^\D/.test(b.firstabc)) return a.firstabc > b.firstabc ? 1 : (a.firstabc == b.firstabc ? 0 : -1);
      return a.firstabc > b.firstabc ? -1 : (a.firstabc == b.firstabc ? 0 : 1);
    })
  })
  return newarr
}