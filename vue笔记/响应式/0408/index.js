...
function defineReactive(obj, key, value) {
    ...
    // 同理，这里的dep同样也是闭包
    var dep = new Dep()
    console.log(dep, 'key:', key, '******defineReactive***************');
    dep.name = key
    Object.defineProperty(obj, key, {
        configurable: true,
        enumerable: true,
        get() {
            console.log('调用了defineReactive的get', dep.name);
            /**
             * depend意义
             */
            dep.depend()
...
class Dep {
    ...
    constructor() {
        console.log('执行了new Dep执行了new Dep执行了new Dep执行了new Dep');
        this.id = depid++;
    ...
    addSub(sub) {
        ...
        if (!this.subids.has(depid)) {
            ...
            this.subs.push(sub);
            console.log(`addSub订阅${this.id}`);
            console.log(this);
        } else {
            console.log(`已经订阅了${this.id}，就不订阅了`);
        }
    ...
    depend() {
        ...
        if (Dep.target) {
            console.log('有Dep.target有Dep.target有Dep.target', Dep.target);
            this.addSub(Dep.target); // 将 当前的 watcher 关联到 当前的 dep 上

            // Dep.target一定是watcher，watcher有addDep方法
            // 这里的this是dep实例
...
class Watcher {
    ...
    addDep(dep) {
        console.log('addDepaddDepaddDep', dep)
        if (!this.depIds.has(watcherid)) {
            console.log('当前dep还没和watcher关联！')
            this.depIds.add(watcherid);
            this.deps.push(dep);
        } else {
            console.log('当前dep已经和watcher关联了！！！！')
        }
...
class Observer {
    constructor(value) {
        ...
        this.dep = new Dep();
        this.ttt = '---';
        console.log('Observer)))))))))))))))))))', value,this);
    ...
    walk(value) {
        ...
        for (let key in value) {
            console.log('let key in valuelet key in value', key);
...
function observe(value) {
    ...
    if (typeof value.__ob__ != 'undefined') {
        console.log(`${value}有ob了`);
        ob = value.__ob__;
    } else {
        console.log(`${value}没有ob，给new一个`);
        ob = new Observer(value)
    }
    console.log('return obreturn obreturn ob', ob)
    return ob
}
...
function getValueByPath(obj, path) {
    ...
    console.log('getValueByPath据路径 访问对象成员', res);
    return res;
}
...
function combine(vnode, data) {
    ...
    if (_type === 3) { // 文本节点 
        ...
        _value = _value.replace(rkuohao, function (_, g) {
            console.log('_value', _value, data, g);
